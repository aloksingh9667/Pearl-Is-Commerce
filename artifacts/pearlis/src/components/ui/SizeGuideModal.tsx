                ...
                <AnimatePresence mode="wait">
                  {tab === "ring" && (
                    <motion.div key="ring" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                      <p className="text-xs text-[#0F0F0F]/50 mb-5 leading-relaxed">
                        Wrap a thin strip of paper around your finger, mark where it overlaps, and measure the length in mm. Match it to the circumference column below.
                      </p>
                      ...
                      <div className="bg-[#D4AF37]/8 border border-[#D4AF37]/20 px-4 py-3 mb-5 flex items-start gap-2">
                        ...
                        <p className="text-xs text-[#0F0F0F]/60 leading-relaxed">Measure at the end of the day when fingers are at their largest. If between sizes, choose the larger size.</p>
                      </div>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {RING_SIZES.map((r, i) => (
                            <tr key={r.in} className={`border-b border-[#0F0F0F]/5 ${i % 2 === 0 ? "bg-white/50" : ""}`}>
                  ...
                  {tab === "bracelet" && (
                    <motion.div key="bracelet" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                      <p className="text-xs text-[#0F0F0F]/50 mb-5 leading-relaxed">
                        Use a soft measuring tape or a strip of paper to measure around your wrist just below the wrist bone. Add 1–2 cm for a comfortable fit.
                      </p>

                      <div className="bg-[#D4AF37]/8 border border-[#D4AF37]/20 px-4 py-3 mb-5 flex items-start gap-2">
                        <span className="text-[#D4AF37] text-xs font-bold shrink-0">TIP</span>
                        <p className="text-xs text-[#0F0F0F]/60 leading-relaxed">For bangles, measure the widest part of your hand (knuckles) when fingers are pressed together.</p>
                      </div>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {BRACELET_SIZES.map((b, i) => (
                            <tr key={b.label} className={`border-b border-[#0F0F0F]/5 ${i % 2 === 0 ? "bg-white/50" : ""}`}>
                  ...
                  {tab === "necklace" && (
                    <motion.div key="necklace" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                      <p className="text-xs text-[#0F0F0F]/50 mb-5 leading-relaxed">
                        Necklace length is measured from end to end including the clasp. The style it creates depends on your neckline and body type.
                      </p>

                      <div className="space-y-3">
                        {NECKLACE_SIZES.map((n, i) => (
                          <div key={n.length} className={`flex items-center gap-4 px-4 py-4 border border-[#0F0F0F]/6 ${i % 2 === 0 ? "bg-white/60" : "bg-transparent"}`}>
